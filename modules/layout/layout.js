 ...
 */
define(
	[ "text!modules/layout/model.json",
		"css!jslibs/yui/2.8.1/resize/assets/skins/sam/resize",
		"css!jslibs/yui/2.8.1/layout/assets/skins/sam/layout",
		"css!modules/layout/layout",
		"!text!modules/layout/layout_tmpl.xml" ],
	function(model, yrscss, ylocss, layoutcss, tmpl) {
	    ...
	    var LayoutView = Backbone.View
			...
			render : function() {
				...
				Logger.error("Layout",
					"render layout with model error." + e);
			    }
			},
			closeAll : function() {
			    this.closeCenter();
			...
			setBackGround : function() {
			    ...
			    $("#" + this.layoutData[0].body).css("background",
				    this.layoutData[0].background);
			    $("#" + this.layoutData[1].body).css("background",
				    this.layoutData[1].background);
			    $("#" + this.layoutData[2].body).css("background",
				    this.layoutData[2].background);
			    $("#" + this.layoutData[3].body).css("background",
				    this.layoutData[3].background);
			},
			setLeftResizeStyle : function() {
			    var elements = $("div[class='yui-resize-handle yui-resize-handle-r']");

			    for ( var i = 0; i < elements.length; i++) {
				$(elements[i]).css("background", "#FFFFFF")
					.css("border", "1px solid #A7A39B");
			    }
			    $("div[class='yui-layout-resize-knob']").attr(
				    "class", "mysprite_resize");
			},
			...
			bindCenterResize : function() {
			    ...
			    this.layout
				    ...
				    .on(
					    'resize',
					    function() {
						var centerWidth = this
							.getSizes().header.w;
						Arbiter
							.publish(
								'layout/center/resized',
								{
								    width : centerWidth
								},
								{
