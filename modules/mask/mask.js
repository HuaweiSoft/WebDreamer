 ...
 */
define([ "text!modules/mask/mask_panel_model.json", "text!modules/mask/mask_panel_tmpl.xml" ], function(model, tmpl) {
    ...
    var init = function() {
        ...
        Arbiter.subscribe(EVENT_MASK_SUBSCRIPE_OPEN, {
            async: true
        }, function(data) // T;
        {
            maskView.open(data);
    ...
    var MaskPanelView = Backbone.View.extend({
        ...
        render: function(tmpl, model) {
            ...
            this.maskPanelText = jsonModel.maskPanelText;
            this.opacity = jsonModel.maskPanelContainerOpacity;
            ...
            if (data.hasOwnProperty("showText")) {
                ...
                if (data.showText) {
                    if (data.hasOwnProperty("text")) {
                        ...
                        $("#" + this.textId).html(this.maskPanelText);
                    }
                    $("#" + this.textId).show();
                } else {
                    $("#" + this.textId).hide();
        ...
        __computeLoadingPanelXY: function(panelTemplateModel) {
            ...
            var panelHeight = parseInt(panelTemplateModel.maskPanelHeight.replace("px", ""), 10);
            var panelWidh = parseInt(panelTemplateModel.maskPanelWidth.replace("px", ""), 10);

            var panelTextWidth = parseInt(panelTemplateModel.maskPanelTextWidth.replace("px", ""), 10);

            panelTemplateModel.maskPanelContainerWidth = windowWidth + "px";
            panelTemplateModel.maskPanelContainerHeight = windowHeight + "px";
            panelTemplateModel.maskPanelTop = (windowHeight - panelHeight) / 2 + "px";
            panelTemplateModel.maskPanelLeft = (windowWidth - panelWidh) / 2 + "px";

            panelTemplateModel.maskPanelTextLeft = (panelWidh - panelTextWidth) / 2 + "px";
