...
define([ "HashMap", "controlBean" ], function(HashMap, Bean) {
    ...
    var modelManager = {
        ...
        assignNewId: function(controlType) {
            ...
            if (lastIndex == -1)
                prefix = controlType[0].toLowerCase() + controlType[0].substring(1);
            else
                prefix = controlType[lastIndex + 1].toLowerCase() + controlType.substring(lastIndex + 2);
            var suffix = 0;
            while (true) {
                suffix++;
        ...
        add: function(bean, trigger) {
            ...
            if (bean.pageNo == null || bean.pageNo <= 0 || bean.pageNo > _pageNumber) {
            ...
            if (bean.pIndex == null || bean.pIndex < 0 || bean.pIndex > maxIndex) {
        ...
        remove: function(id, trigger) {
            ...
            for ( var j = 0; j < _beans.length; j++) {
                if (_beans[j].id != id && _beans[j].pageNo == bean.pageNo && _beans[j].pIndex > bean.pIndex) {
                    _beans[j].pIndex -= 1;
                }
            }
            genIdIndexMap();
        ...
        removePage: function(pageNo, trigger) {
            ...
            for ( var i = 0; i < _beans.length - deleted; i++) {
                ...
                if (bean.pageNo == pageNo) {
                    ...
                    _beans.splice(i, 1);
                    i--;
                    // use EVENT_CONTROL_REMOVE_PAGE event to check all deleted controls
                    /*
                     * Arbiter.publish(EVENT_CONTROL_REMOVE, { id : bean.id, bean : bean, trigger :
                     * trigger });
                     */
        ...
        reorder: function(id, newPageIndex) {
            ...
            this.forEach(function(obj) {
                if (obj != bean && obj.pageNo == bean.pageNo) {
                    if (currentIndex > newPageIndex) {
                        if (obj.pIndex >= newPageIndex && obj.pIndex < currentIndex)
                            obj.pIndex++;
                    } else if (currentIndex < newPageIndex) {
                        if (obj.pIndex > currentIndex && obj.pIndex <= newPageIndex)
                            obj.pIndex--;
                    }
                }
            });
    ...
    function init() {
        ...
        Arbiter.subscribe(EVENT_MODEL_UPDATE, function(params) {
            modelManager.setPropValue(params.id, params.propName, params.propValue, modelManager.NAME);
        });
        ...
        Arbiter.subscribe(EVENT_MODEL_REORDER, function(params) {
            modelManager.reorder(params.id, params.newPageIndex, modelManager.NAME);
        });
