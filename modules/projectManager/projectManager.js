...
define(
    [ "css!modules/projectManager/projectManager", "text!modules/projectManager/project_manager_tmpl.xml",
        ...
        "flowDesigner", "ide" ],
    function(css, tmpl, itemtmpl, noProTipTmpl, createProTmpl, $, util, modelManager, flowDesigner, ide) {
        ...
        var ProjectMgrPanelView = Backbone.View.extend({
            user: "",
            currentProjectName: "",
            ...
            setCurrentProjectName: function(projectName) {
                ...
                PROJECT_PATH = "data/" + this.user + "/" + projectName + "/";
                ide.projectName = projectName;
                ide.projectPath = PROJECT_PATH;
        ...
        var projectService = {
            ...
            _loadUiModel: function(userId, projectName, successCallback, failCallback) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        var next = function() {
                            modelManager.load(beans, pageNumber);
                            if (successCallback)
                                successCallback(result);
                        };
                        if (ide.isControlLibLoaded) {
                            next();
                        }
                        else {
                            //check whether all control js used in app have been loaded
                            var allLoaded = true;
                            for (var i = 0; i < beans.length; i++) {
                                var type = (beans[i].type || "").trim();
                                if (!type)
                                    continue;
                                var creator = util.getObjValueByJPath(window, type);
                                if (!creator) {
                                    console.debug("[%s] control js haven't been loaded.", type);
                                    allLoaded = false;
                                    break;
                                }
                            }
                            ...
                            else if (ide.isLoadingControlLib) {
                                _this._subscribeControlJsLoadFinishEvent(next);
                            }
                            else {
                                var subscribeId = Arbiter.subscribe(EVENT_BEGIN_LOAD_ALL_CONTROL_JS, function() {
                                    Arbiter.unsubscribe(subscribeId);
                                    _this._subscribeControlJsLoadFinishEvent(next);
                                });
                            }
                        }
            ...
            _subscribeControlJsLoadFinishEvent: function(callback){
                var subscribeId = Arbiter.subscribe(EVENT_FINISH_LOAD_ALL_CONTROL_JS, function(){
                    Arbiter.unsubscribe(subscribeId);
                    callback();
                });
            },
